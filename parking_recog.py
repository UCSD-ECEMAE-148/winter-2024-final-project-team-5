...
class RoboflowOakNode(Node):
    ...
    def run(self):
        ...
        while rclpy.ok():
            ...
            # Check for detections with confidence > 0.3
            ...
            # If detection made with confidence > 0.3, run the second node
            if detection_made:
                self.run_second_node()
                break
    ...
    def run_second_node(self):
        # Run the second ROS node using subprocess
        subprocess.Popen(["ros2", "launch", "ucsd_robocar_nav2_pkg", "all_nodes.launch.py"])
